...
type ProcedureRoomOrdersTab = { queue: string };
const ProcedureOrdersTabs: React.FC<ProcedureRoomOrdersTab> = ({ queue }) => {
  const { t } = useTranslation();
  const [selectedTab, setSelectedTab] = useState(0);
  const tabExtensions = useAssignedExtensions(procedurePanelSlot);

  // Get individual statistics each tab using their fulfillerstatus
  const activeOrdersStats = useProcedureOrderStats('', queue);
  const inProgressStats = useProcedureOrderStats('IN_PROGRESS', queue);
  const referredStats = useProcedureOrderStats('EXCEPTION', queue);
  const notDoneStats = useProcedureOrderStats('DECLINED', queue);
  const completedStats = useProcedureOrderStats('COMPLETED', queue);
      ...
      <Tabs selectedIndex={selectedTab} onChange={({ selectedIndex }) => setSelectedTab(selectedIndex)}>
        ...
        <TabPanels>
          <TabPanel style={{ padding: 0 }}>
            <ProcedureOrderedList fulfillerStatus="NEW" queue={queue} />
          </TabPanel>
          {tabExtensions
            .filter((extension) => Object.keys(extension.meta).length > 0)
            .map((extension, index) => {
